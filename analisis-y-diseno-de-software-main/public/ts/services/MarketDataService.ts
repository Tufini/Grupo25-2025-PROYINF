...
export interface CompetitorSimulation {
...
const BANK_SPREADS_OVER_TPM: Record<string, number> = {
...
class MarketDataService {
     ...
     */
    async getUserLocation(): Promise<{ countryCode: string, countryName: string }> {
     ...
     */
    async getEconomicIndicators(countryCode: string): Promise<EconomicIndicators> {
     ...
     */
    async getAvailableCompetitors(countryCode: string): Promise<Competitor[]> {
     ...
     */
    async simulateCompetitor(
        competitorId: string,
        amount: number,
        months: number,
        countryCode: string
    ): Promise<CompetitorSimulation> {
        await new Promise(resolve => setTimeout(resolve, 500));
    ...
    async getSmartScenarios(
        ...
        if (conservativeMonths === baseMonths) conservativeMonths = Math.max(6, baseMonths - 6);
        ...
        if (riskyMonths === baseMonths) riskyMonths = Math.min(60, baseMonths + 12);
    ...
    async generateCounterOffers(
        ...
        if (conservativeMonths === months) conservativeMonths = Math.max(6, months - 6);
        ...
        if (riskyMonths === months) riskyMonths = Math.min(60, months + 12);
