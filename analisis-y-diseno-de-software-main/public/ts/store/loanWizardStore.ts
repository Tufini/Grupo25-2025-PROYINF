...
export interface LoanWizardState {
  ...
  plazo: number;
  tasa: number;
  tipo_credito: 'CONSUMO' | 'HIPOTECARIO' | 'AUTOMOTRIZ';
  ...
  // Acciones
  setFormData: (data: Partial<LoanWizardState>) => void;
  setSeguroDesgravamen: (value: boolean) => void;
  setSeguroCesantia: (value: boolean) => void;
  setPrimerVencimiento: (date: string | null) => void;
