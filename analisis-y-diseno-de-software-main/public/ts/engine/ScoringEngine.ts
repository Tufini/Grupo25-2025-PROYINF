...
export type ContractType = 
  ...
  | 'INDEPENDIENTE'   // Trabajador independiente/boleta de honorarios
  | 'TEMPORAL';       // Temporal o por proyecto

/**
 * Estado del resultado de evaluación
 */
export type RiskStatus = 
  | 'APROBADO'              // Aprobación estándar
  | 'APROBADO_PREMIUM'      // Aprobación con tasa preferencial
  | 'CONDICIONAL'           // Aprobación con condiciones (monto reducido o aval)
  | 'RECHAZADO';            // Solicitud rechazada

/**
 * Nivel de historial crediticio simulado
 */
export type CreditHistory = 
  | 'IMPECABLE'             // Sin atrasos ni incidencias
  | 'ATRASOS_OCASIONALES'   // Algunos atrasos menores
  | 'PROBLEMAS_MENORES'     // Problemas ocasionales
  | 'DESCONOCIDO';          // Sin historial previo
...
export interface UserData {
  ...
  ingresoLiquido: number;                 // Ingreso líquido mensual en CLP
  otrasDeudas: number;                    // Suma de cuotas mensuales de otras deudas en CLP
  tieneDeudaMorosa: boolean;              // Indica si tiene deudas impagas en DICOM
  tipoContrato: ContractType;             // Tipo de contrato laboral
  antiguedadLaboral: number;              // Antigüedad en el trabajo actual (meses)
  esPropietario: boolean;                 // Propietario de bien raíz o vehículo
  esProfesional: boolean;                 // Posee título profesional universitario
  historialCrediticio?: CreditHistory;    // Historial crediticio (opcional, se simula si no existe)
}
...
export interface LoanRequest {
  montoSolicitado: number;                // Monto del crédito solicitado en CLP
  plazoMeses: number;                     // Plazo en meses
  tasaMensual?: number;                   // Tasa de interés mensual (opcional, default: 1.1%)
}
...
export interface RiskFactor {
  codigo: string;                         // Código identificador del factor
  descripcion: string;                    // Descripción del factor de riesgo
  impacto: 'CRITICO' | 'ALTO' | 'MEDIO' | 'BAJO'; // Nivel de impacto
}
...
export interface ScoreBreakdown {
  cargaFinanciera: {
    puntos: number;
    maximo: number;
    rci: number;                          // Relación Cuota/Ingreso (%)
    detalle: string;
  };
  estabilidadLaboral: {
    puntos: number;
...
export interface RiskResult {
  ...
  status: RiskStatus;                     // Estado de la evaluación
  maxAmount: number;                      // Cupo máximo aprobado en CLP
  montoAprobado: number;                  // Monto aprobado para esta solicitud
  tasaOfrecida: number;                   // Tasa mensual ofrecida (%)
  cuotaEstimada: number;                  // Cuota mensual estimada en CLP
  riskFactors: RiskFactor[];              // Factores de riesgo identificados
  breakdown: ScoreBreakdown | null;       // Desglose del score (null si fue rechazado por knock-out)
  mensaje: string;                        // Mensaje explicativo del resultado
  recomendaciones: string[];              // Recomendaciones para el solicitante
}
...
export class ScoringEngine {
   ...
   */
  public static evaluateRisk(userData: UserData, loanRequest: LoanRequest): RiskResult {
    ...
    const cuotaMensual = this.calculateMonthlyPayment(
      loanRequest.montoSolicitado,
      tasaMensual,
      loanRequest.plazoMeses
    );

    // FASE 3: CÁLCULO DEL SCORE POR COMPONENTES
    const cargaFinanciera = this.evaluateCargaFinanciera(
      cuotaMensual,
      userData.otrasDeudas,
      userData.ingresoLiquido
    );

    const estabilidadLaboral = this.evaluateEstabilidadLaboral(
      userData.tipoContrato,
      userData.antiguedadLaboral
    );

    const patrimonioYPerfil = this.evaluatePatrimonioYPerfil(
      userData.esPropietario,
      userData.esProfesional
    );

    const historialCrediticio = this.evaluateHistorialCrediticio(
      userData.historialCrediticio
    );

    // FASE 4: SCORE TOTAL
    const scoreTotal = 
      cargaFinanciera.puntos +
      estabilidadLaboral.puntos +
      patrimonioYPerfil.puntos +
      historialCrediticio.puntos;

    // FASE 5: CÁLCULO DE CAPACIDAD MÁXIMA
    const capacidadMaxima = this.calculateMaxCapacity(
      userData.ingresoLiquido,
      userData.otrasDeudas,
      loanRequest.plazoMeses,
      tasaMensual
   ...
   */
  private static checkKnockoutConditions(userData: UserData): RiskResult | null {
   ...
   */
  private static evaluateCargaFinanciera(
   ...
   */
  private static evaluateEstabilidadLaboral(
   ...
   */
  private static evaluatePatrimonioYPerfil(
   ...
   */
  private static evaluateHistorialCrediticio(historial?: CreditHistory) {
  ...
  public static calculateMaxCapacity(
    ...
    otrasDeudas: number,
    plazoMeses: number,
    tasaMensual: number = TASA_ESTANDAR
  ): number {
  ...
  private static determineResult(
    ...
    let montoAprobado: number;
    let tasaOfrecida: number;
    let mensaje: string;
    ...
    // CASO 2: SCORE 550-699 - APROBACIÓN CONDICIONAL
      ...
      const montoPropuesto = Math.min(
        montoSolicitado * FACTOR_REDUCCION_CONDICIONAL,
        capacidadMaxima
      );
   ...
   */
  private static simulateCreditHistory(): CreditHistory {
   ...
   */
  public static formatResultForDebug(result: RiskResult): string {
    const lines: string[] = [
      '='.repeat(80),
